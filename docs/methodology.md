...
"Agent memory" is an overloaded label; products under that name are in fact solving completely different problems. Decompose them along this kit's four-role coordinate system and the differences become immediately clear (the first three examples were collected 2026-06-18~19 from Hacker News / X; the fourth and fifth were read at the source level on 2026-08-01 and 2026-08-07 respectively):
...
| **qm** (github.com/yc-software/qm, MIT) | **durable facts about a person or a room** (who is who, what they are working on, how they like to work) — one markdown notebook, hard-capped at 300 bullets | **the full librarian** (per-turn extraction + periodic consolidation) **plus a write-side scope constraint**; **no critic at all**, and no retrieval — the notebook's tail is injected whole | open source / YC official; the memory layer is 680 lines inside a 140k-line system |
| **TencentDB-Agent-Memory** (github.com/TencentCloud, MIT) | **four memory-asset classes shared by a team of agents**: distilled conversations (layered L0→L3) / skills / wiki / code graph, bound per-agent via an "equipment list" | **librarian + store + retrieval maximized** (five-stage extraction, conflict-decision quadruple, recall budgets, ACL/multi-tenancy); **no critic, no evolve** — prompts are hard-coded constants, and recall ignores recency | open source / 155k lines of TS; tests stripped from the release |

Five things this validates about this kit's design:
...
4. **qm draws the sharpest line of the four, because it is the only one whose source can be read.** It also has no critic — a fourth vote for the pattern above — but the reason is not that it cut a corner: **qm and this kit remember different kinds of thing.** qm remembers *facts about* a person and the world; this kit remembers *lessons learned* about how the agent should do better. That single difference propagates into every downstream decision. The number of durable facts about a person is **bounded**, so qm can hard-cap the notebook at 300 bullets, consolidate it periodically, and then inject the whole tail — no retrieval needed. The number of lessons grows **unbounded** with the task space, so this kit must filter on the read side with top-k. Stated once: **"periodic consolidation" and "semantic retrieval" are two solutions to the same problem** — one compresses on write, the other filters on read. Pick by whether your memory is bounded. What qm gives this kit is not a reason to add a critic, but a set of write-side gates for the `librarian` stub that no closed-source product could have shown us: untrusted-provenance rewriting, an extraction contract that forbids deriving a preference from the assistant's own reply, a demotion rule for autonomous (nobody-spoke) turns, and consolidation that emits structured actions instead of rewriting the notebook. Those four are now recorded in `librarian/README.md` and `reflector/README.md`.
5. **TencentDB-Agent-Memory is this kit's mirror image.** It industrializes librarian+store+retrieval (155k lines) yet contains no critic and no evolve anywhere — prompts are hard-coded constants, there is no judge, no "did this memory help next time" loop; conversely its recall ignores recency and stores a confidence field it never uses for ranking — exactly the two things this kit's retrieval already does. **Each is the other's hole**, which is more informative than a third vote for "big shops also cut the critic": even the best-resourced implementation maxed out the deposition side rather than the loop side, suggesting critic/evolve is hard because it requires *defining a quality yardstick*, not because of engineering. Five designs were ported back and have landed: warm-up escalating trigger + idle fallback (`reflector/trigger.py`), the conflict-decision quadruple (MCP `memory_write` dedup gate), dual recall budgets with truncation-that-points-to-the-tool (`retrieval`), cache-aware injection split (`MEMORY.md` startup protocol), and the classify-then-score dual write gate (`librarian/README.md` + steering).
